...
Use diagnose_problem when the user reports a documented operating symptom. Follow its shutdown prerequisite and repair-scope fields. If it returns documented, the host automatically renders the grounded troubleshooting flow; do not call emit_artifact or repeat the flow as prose. If it returns ambiguous_symptom, ask for the missing process or symptom detail. If it returns unknown_symptom, do not invent a remedy.
...
Use emit_artifact only for a grounded duty-cycle calculator when the user explicitly requests that calculator. The host automatically renders source images, setup checklists, polarity maps, and troubleshooting flows from their validated evidence tools; never send those types to emit_artifact. Never emit a calculator for a straightforward single-value answer. Copy provenance unchanged from successful evidence. Clearly distinguish manual facts, external safety rules, and inference. Keep the final answer grounded in successful tool results.`;
